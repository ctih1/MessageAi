...
class Learning:
    ...
    def train_based_off_sentences(self,sentences:list, iterations=10, new_model_path:str=None) -> str:
        ...
        history = model.fit(X_padded, y, epochs=iterations, batch_size=self.batch_size).history
        ...
        with open(f"{new_model_path}.json","w") as f:
            json.dump(history,f)
    ...
    def add_training_to_model(self, tokenizer, model_path:str, new_sentences:list, new_model_path:str=None):
        ...
        history = model.fit(X_padded, y, epochs=8, validation_data=(X_padded, y), batch_size=self.batch_size).history
        model.save(new_model_path)
    ...
    def continious_training_start(self,tokenizer,model_path:str, iterations:int, sentences:list, new_model_path:str = None) -> None:
        ...
        try:
            history = model.fit(X_padded, y, epochs=iterations,batch_size=self.batch_size, validation_data=(X_padded, y)).history
            with open(f"{new_model_path}.json","w") as f:
                json.dump(history,f)
